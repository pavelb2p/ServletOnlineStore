...
<div class="container">
    <a href="changeLocale"><%=changeLocale%>
    </a>
    <div class="d-flex py-3">
        <h3><fmt:message key="cart.totalprice" bundle="${lang}"/> $ ${(total>0)?total:0}</h3>
        <a class="mx-3 btn btn-primary" href="/buyAllTargetServlet"><fmt:message key="cart.checkOut"
                                                                                 bundle="${lang}"/></a>
    </div>
            ...
            <td class="main-cell">
                <form action="order-now" method="post" class="form-inline">
                    ...
                    <div class="form-group d-flex justify-content-between w-20">
                        ...
                        <input style="width:40px" type="text" name="quantity" class="form-control"
                        ...
                        <button type="submit" class="btn btn-primary btn-sm w-20">
                            <fmt:message key="cart.buy.button" bundle="${lang}"/></button>
                    </div>
                </form>
            </td>
            <td><a class="button btn-sm btn-danger" href="remove-from-cart?id=<%= c.getId() %>"> <fmt:message
                    key="cart.remove.button" bundle="${lang}"/></a></td>
        </tr>
        <% }
        }
        %>
